...
// 新增仓库弹窗组件
const AddWarehouseModal = memo(({visible, handleCancel, handleOk, ref}: AddWarehouseModalProps) => {
  // 仓库类型
  type FileType = {
    name: string,
    type: string,
  ...
  return(
    ...
    >
      <Form
        ...
        autoComplete="off"
        form={form}>
        <Form.Item label="仓库名称" name="name" rules={[{ required: true, min: 2, max: 20, message: '请输入2-20个字符的仓库名称' }]}>
          <Input placeholder="请输入仓库名称" maxLength={20} />
        </Form.Item>
        <Form.Item label="仓库类型" name="type" rules={[{ min: 2, max: 8, message: '请输入2-8个字符的仓库类型' }]}>
          <Input placeholder="请输入仓库类型" maxLength={8} />
        </Form.Item>
        <Form.Item label="仓库描述" name="description" rules={[{ min: 0, max: 200, message: '请输入0-200个字符的仓库描述' }]}>
